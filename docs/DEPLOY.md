...
## Шаг 10 — Ежедневные отчёты в Telegram

1. В Vercel добавь переменные:
   - `CRON_SECRET` (любой длинный случайный секрет)
   - `REPORT_TZ` (например `Asia/Almaty`)
2. Cron уже настроен в `vercel.json` на ежедневный запуск.
3. При необходимости поменяй время в `vercel.json` (формат cron, время в UTC).

Запуск отправляет отчёт за вчерашний день всем активным пользователям.
